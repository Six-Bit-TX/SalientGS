...
We ablate each key component of SalientGS on the Mip-NeRF 360 benchmark to quantify individual contributions. All variants share the same first-order SfM initialization and Fisher Vector matching; only the 3DGS training stage differs unless otherwise noted. These controlled ablations were run as a matched experiment set under the camera-ready software snapshot; their within-table deltas isolate component effects. Table~\ref{tab:main-result}, by contrast, is the later released-code verification. Absolute values across the two experiment sets should therefore not be subtracted from one another.
...
\begin{table}[t]
    ...
    \caption{
        \textbf{Controlled component ablation on Mip-NeRF 360.} $\Delta$PSNR is relative to the matched full configuration.
    }
    \begin{tabular}{l ccc r}
        ...
        \quad Controlled full & \textbf{28.89} & \textbf{0.857} & \textbf{0.131} & --- \\
...
Against matched vanilla MCMC, guidance adds 0.17~dB and lowers LPIPS by 12\% (0.149$\to$0.131). Birth guidance provides the larger contribution; relocation is most useful when birth is also guided. Removing footprint normalization costs 6.19~dB because large projected primitives otherwise dominate the scores.

\paragraph{Densification Strategy (Group B)}
Replacing guided MCMC management with standard ADC costs 1.22~dB, supporting multi-view redistribution under a fixed budget.
...
\begin{table}[t]
    ...
    \setlength\tabcolsep{1.5pt}
    \caption{
        \textbf{Controlled pose/SfM ablation} on Mip-NeRF 360. $\Delta$PSNR is relative to the matched full configuration.
    }
    \begin{tabular}{l ccc r}
        \toprule
        Configuration & PSNR$\uparrow$ & SSIM$\uparrow$ & LPIPS$\downarrow$ & $\Delta$PSNR \\
        ...
        \quad Controlled full ($k{=}20$ retrieval) & \textbf{28.89} & 0.857 & \textbf{0.131} & --- \\
...
Removing BA costs 0.19~dB, whereas freezing poses costs 0.57~dB. Thus photometric refinement provides most of the recovery and reprojection anchoring adds a further measurable gain.

\paragraph{SfM Initialization Quality (Group B)}
Reducing retrieval from $k{=}20$ to 10 and 5 costs 1.54 and 2.32~dB, respectively; at $k{=}5$, \textit{stump} fails at 14.97~dB. Joint optimization therefore cannot replace a connected, reliable view graph.
...
Guidance improves every tested budget, with the gain shrinking from +0.27~dB at 500K to +0.09~dB at 3M. Guided 1M (28.81~dB) already exceeds vanilla 1.5M (28.72~dB), showing the largest value under tight capacity.

\paragraph{Schedule and normalization sensitivity}
In the controlled sweep, $T{=}500$, a 3K warmup, and robust quantiles perform best; the supplementary material reports every setting. These matched sensitivity values are distinct from the release-verification aggregate in Table~\ref{tab:main-result}.
