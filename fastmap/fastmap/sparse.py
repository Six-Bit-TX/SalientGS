...
def sparse_reconstruction(
    ...
    # Use a mask to avoid division by zero instead of adding epsilon
    valid_weight_mask = weights > 0  # (num_tracks,)
    xyz[valid_weight_mask] /= weights[valid_weight_mask].unsqueeze(-1)
    # Set invalid points to NaN so they can be filtered out later
    xyz[~valid_weight_mask] = float('nan')
    del weights
    ...
    ##### Filter out 3D points with NaN or Inf coordinates #####
    valid_points3d_mask &= torch.isfinite(xyz).all(dim=-1)  # (num_tracks,)
    ...
    # fetch the keypoint mask, image idx, keypoint idx, and xy_pixels for each 2D point from GPU to lists on CPU
    ...
    xy_pixels_list = tracks.xy_pixels.tolist()  # List[List[float]] (num_points2d, 2)
    ...
    for i in range(tracks.num_tracks):
        ...
        # get the image indices, keypoint indices, and xy pixel coordinates
        track_image_idx = [
            x
        ...
        track_xy_pixels = [
            tuple(xy)
            for xy, mask_value in zip(xy_pixels_list[start_idx:end_idx], mask_list)
            if mask_value
        ]
        assert len(track_image_idx) == len(track_keypoint_idx) == len(track_xy_pixels)
        ...
        container.track_xy_pixels.append(track_xy_pixels)
    ...
    assert container.num_points == len(container.track_xy_pixels)
